...
5. **WEB_APP_URL (опционально):**
   - Railway автоматически определяет URL через `RAILWAY_PUBLIC_DOMAIN`
   - Код автоматически использует его, дополнительная настройка не требуется
   - Если используете свой домен, добавьте `WEB_APP_URL` в Variables
...
| `WEB_APP_URL` | URL вашего приложения (опционально, Railway определяет автоматически) | ❌ Нет |
